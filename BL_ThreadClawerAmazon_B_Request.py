...
class ThreadClawerAmazon(Thread):
        ...
        try:
            ...
            if product_description:
                product_info['product_description'] = etree.tostring(product_description[0], encoding="utf-8",
                                                                     pretty_print=True, method="html").decode('utf-8')
            else:
    ...
    def clawer(self, product_link):
        try:
            ...
            # 主体商品
            product_html, title, productLink = self.__request__(productLink)
            if title == 'Robot Check':
                print('Robot Check--1')
                product_html = self.get_character_by_ocr(productLink)
            elif title == 'Sorry! Something went wrong!':
                print('Sorry! Something went wrong!--1')
                self.product_link_queue.put(productLink)
                return
...
class GetAllProductsLink(object):
    ...
    def __getProductlink__(self, url):
        try:
            ...
            products_list = html.xpath('//li[contains(@id,"result_")]')
            if not products_list:
                products_list = html.xpath('//div[@class="sg-col-20-of-24 s-result-item sg-col-0-of-12 sg-col-28-of-32 sg-col-16-of-20 sg-col sg-col-32-of-36 sg-col-12-of-16 sg-col-24-of-28"]')
    ...
    def __clawer__(self, url):
        ...
        try:
            ...
            for product in products_list:
                protuct_link_sub = str(product.xpath('.//a[@class="a-link-normal a-text-normal"]/@href')[0]) + '&language=en_US'
                if "www.amazon.com" in protuct_link_sub:
                    protuct_link = protuct_link_sub
                else:
                    protuct_link = 'https://www.amazon.com' + protuct_link_sub
                price = str(product.xpath('string(.//span[@class="a-offscreen"])')).replace(' ', '')
                if not price:
                    price = product.xpath('./div/div/div/div[2]/div[2]/div[1]/div/div/a/span[2]')
                    if price:
                        price = str(product.xpath('./div/div/div/div[2]/div[2]/div[1]/div/div/a/span[2]/text()')[0]).replace(' ', '')
                    else:
                        price = ''
                self.product_link_queue.put([protuct_link, price])
    ...
    def __getNextPage__(self, html):
        try:
            ...
            if next_:
                next_url = 'https://www.amazon.com' + str(next_[0])
                return next_url
            else:
                next_ = html.xpath('//a[@id="pagnNextLink"]/@href')
                if next_:
                    next_url = 'https://www.amazon.com' + str(next_[0])
                    return next_url
                else:
    ...
    def __request__(self, store_link):
        try:
            ...
            headers = {
                ...
                # "Accept-Encoding": "gzip, deflate, br",
                "Accept-Language": "zh-CN,zh;q=0.9",
                "Cookie": 'i18n-prefs=USD; sp-cdn="L5Z9:HK"; x-wl-uid=1AN7TJNCV5Qcv++n7laTmt5KmoDqjJ534vOHCKWbNTSBe/uhX6z8Qwh5iDFms/D1sK3+oUCEagSo=; x-amz-captcha-1=1566912462828647; x-amz-captcha-2=+E/Lzh4WuSxTn2gb+yzQPw==; lc-main=en_US; session-token=BrYwjo73uMxxnHNXGzlvRIcDg0dVDRRgmMC3dR8vFkha+q4h6RsELJ9qCqs96vm9drKTAYnmsET28zx6N+Zf/SNORT4zaWPaWLPOzB6fTneoGOa9DKqzD/UcCwayNndie6pJZYaT0CQ9y9WDLs40NlVHZbPpOeSudb14Fmbk4xXaRhXHEP+XtZc40oOJjqXt; ubid-main=133-3456038-7774832; session-id-time=2082787201l; session-id=136-8826518-8862022; csm-hit=tb:WTSW2PSZGYZ914MV79ES+s-MV7VGRTZTJ1ZQCC4BF9Z|1566977636892&t:1566977636892&adb:adblk_no'
            }
...
if __name__ == '__main__':
    ...
    url = 'https://www.amazon.com/fellt/b/ref=w_bl_sl_ap_ap_web_13459008011?ie=UTF8&node=13459008011&field-lbr_brands_browse-bin=fellt'
